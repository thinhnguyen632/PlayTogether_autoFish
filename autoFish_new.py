...
def detectFish(gray):
    ...
    # low_s, high_s = (50, 250)
    # low_v, high_v = (50, 250)
    # mask = cv.inRange(gray, (low_h, low_s, low_v), (high_h, high_s, high_v))
    h, s, v = cv.split(gray)
    # mask = cv.adaptiveThreshold(h, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 5, 2) # can vit
    mask = cv.adaptiveThreshold(h, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 9, 1) # can meo
...
while(True):
    ...
    img_fish = cv.cvtColor(img_fish, cv.COLOR_BGR2HSV)
